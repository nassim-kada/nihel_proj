...
function StepBar({ current }: { current: number }) {
  const steps = ["Informations personnelles", "Diplômes & Spécialités", "Détails cabinet"];
  return (
...
export default function DoctorSignupPage() {
  ...
  const uploadFile = async (file: File, onDone: (url: string) => void, setUploading: (b: boolean) => void) => {
    ...
    try {
      ...
      else setMessage({ type: "error", text: "Erreur lors de l'upload du fichier." });
    } catch {
  ...
  const validateStep = (i: number) => {
    if (i === 0) {
      if (!s1.firstName || !s1.email || !s1.phone || !s1.wilaya || !s1.password) return "Veuillez remplir tous les champs obligatoires.";
      if (s1.password !== s1.confirmPassword) return "Les mots de passe ne correspondent pas.";
    }
    if (i === 1) {
      if (!s2.specialty) return "Veuillez sélectionner une spécialité.";
      if (!s2.medicalOrderNumber) return "Le numéro d'ordre médical est obligatoire.";
    }
    if (i === 2) {
      if (!s3.clinicName || !s3.clinicAddress || !s3.clinicWilaya || !s3.clinicPhone) return "Veuillez remplir les champs obligatoires de la clinique.";
      if (!s3.acceptTerms) return "Vous devez accepter les conditions d'utilisation.";
    }
    return null;
  };
    ...
    <main className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-sky-50 flex items-center justify-center py-10 px-4">
      <div className="w-full max-w-2xl">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500 mt-1">Ravi de vous voir. Créez votre espace médecin.</p>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
            ...
            <div className="space-y-4">
              ...
              <Field label="Type">
                <div className="flex gap-2">
                  {(["Médecin", "Clinique"] as const).map(t => (
                    <button key={t} type="button" onClick={() => setS1(p => ({ ...p, type: t }))}
                      className={`flex-1 py-2.5 rounded-lg text-sm font-semibold border transition-all ${s1.type === t ? "bg-blue-600 text-white border-blue-600" : "border-gray-200 text-gray-600 hover:border-blue-300"}`}>
                      {t}
                    </button>
                  ))}
                </div>
              ...
              <Field label="Hôpital / Établissement">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Genre">
                  <div className="flex gap-2 mt-0.5">
                    {["Homme", "Femme"].map(g => (
                      <label key={g} className={`flex-1 flex items-center justify-center py-2.5 rounded-lg border cursor-pointer text-sm font-medium transition-all ${s1.gender === g ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 text-gray-600 hover:border-blue-300"}`}>
                        <input type="radio" name="gender1" value={g} checked={s1.gender === g} onChange={() => setS1(p => ({ ...p, gender: g }))} className="sr-only" />
                        {g}
                      </label>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Confirmez le mot de passe" required>
                  <div className="relative">
          ...
          {step === 1 && (
            <div className="space-y-5">
              <h2 className="text-lg font-bold text-gray-800 mb-4">Diplômes & Spécialités</h2>

              {/* Specialty search */}
              <Field label="Spécialité principale" required>
                <input value={s2.specialtySearch} onChange={e => setS2(p => ({ ...p, specialtySearch: e.target.value }))}
                  placeholder="Rechercher une spécialité..." className={`${inputCls} mb-2`} />
                <div className="border border-gray-200 rounded-lg max-h-36 overflow-y-auto">
                  {filteredSpecialties.length === 0 ? (
                    <p className="text-sm text-gray-400 p-3">Aucune spécialité trouvée</p>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs font-semibold text-gray-600">Diplôme(s) obtenus <span className="text-red-500">*</span></label>
                  ...
                  <div key={i} className="p-3 bg-gray-50 rounded-lg border border-gray-200 mb-2 space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      <input value={d.title} onChange={e => setDiplomas(prev => prev.map((x, j) => j === i ? { ...x, title: e.target.value } : x))}
                        placeholder="Titre du diplôme" className={inputCls} />
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Numéro d'ordre médical" required>
                  <div className="relative">
              ...
              {/* Diploma upload */}
              <Field label="Upload scan du diplôme / certificat">
                <label className={`flex items-center gap-3 px-4 py-3 border-2 border-dashed rounded-lg cursor-pointer transition-all ${uploadingDiploma ? "border-blue-300 bg-blue-50" : "border-gray-300 hover:border-blue-400 hover:bg-blue-50"}`}>
                  ...
                  <span className="text-sm text-gray-600">{s2.diplomaFileUrl ? "✓ Fichier uploadé" : "Cliquez pour uploader (PDF ou image)"}</span>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <h2 className="text-lg font-bold text-gray-800 mb-4">Détails de la clinique / cabinet</h2>

              <Field label="Nom de la clinique / cabinet" required>
                <div className="relative">
                  <Building2 className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              ...
              <Field label="Numéro de téléphone de la clinique" required>
                <div className="relative">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs font-semibold text-gray-600 flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> Horaires de consultation</label>
              ...
              {/* Clinic photo */}
              <Field label="Photo de la clinique (optionnel)">
                <label className={`flex items-center gap-3 px-4 py-3 border-2 border-dashed rounded-lg cursor-pointer transition-all ${uploadingPhoto ? "border-blue-300 bg-blue-50" : "border-gray-300 hover:border-blue-400 hover:bg-blue-50"}`}>
                  ...
                  <span className="text-sm text-gray-600">{s3.clinicPhotoUrl ? "✓ Photo uploadée" : "Cliquez pour uploader"}</span>
          ...
          <div className="flex items-center justify-between mt-8 pt-5 border-t border-gray-100">
            {step > 0 ? (
              ...
              <Link href="/login" className="text-sm text-gray-400 hover:text-gray-600">Déjà un compte</Link>
            )}
        ...
        <p className="text-center text-xs text-gray-400 mt-4">
          ...
          <Link href="/signup/patient" className="text-teal-600 font-semibold hover:underline">Inscription patient</Link>
        </p>
      </div>
    </main>
  );
