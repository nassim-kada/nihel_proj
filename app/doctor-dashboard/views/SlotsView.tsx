...
const SlotsView: FC<{ currentDoctorId: string }> = ({ currentDoctorId }) => {
  ...
  return (
    <div className="space-y-6 max-w-xl">
      <h2 className="text-xl font-bold text-gray-800">{t("doctor_dashboard.slots.title", "Mes Disponibilités")}</h2>
      <form onSubmit={handleAdd} className="space-y-3">
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: t("doctor_dashboard.slots.date", "Date"), type: "date", value: date, set: setDate },
            { label: t("doctor_dashboard.slots.start", "Début"), type: "time", value: startTime, set: setStartTime },
            { label: t("doctor_dashboard.slots.end", "Fin"), type: "time", value: endTime, set: setEndTime },
          ].map(f => (
            <div key={f.label}>
        ...
        <button type="submit" className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-sky-500 text-white rounded-lg font-semibold text-sm shadow transition-all">
          <PlusSquare className="w-4 h-4" /> {t("doctor_dashboard.slots.add", "Ajouter")}
        </button>
      </form>
      <div className="space-y-2">
        <p className="text-sm font-semibold text-gray-600">{t("doctor_dashboard.slots.configured_slots", "Plages configurées")} ({slots.length})</p>
        {slots.length === 0 ? (
          <p className="text-sm text-gray-400 italic py-4 text-center">{t("doctor_dashboard.slots.no_slots", "Aucune plage configurée.")}</p>
        ) : slots.map(s => (
          <div key={s._id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-200">
            <div>
              ...
              <p className="text-xs text-gray-500">{s.times[0]} → {s.times[s.times.length - 1]} · {s.times.length} {t("doctor_dashboard.slots.slots_count", "créneaux")}</p>
            </div>
